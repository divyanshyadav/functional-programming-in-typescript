...
describe('Maybe', () => {
    it('should return the same container if value is null', () => {
        const container: Maybe = new Maybe(null)
        expect(container.map(a => a)).toBe(container)
    })

    it('should return the same container if value is null', () => {
        const container: Maybe = new Maybe(undefined)
        expect(container.map(a => a)).toBe(container)
    })

    it('should return new container if value is not null or undefined', () => {
        const container: Maybe = new Maybe(1)
    ...
    it('should not return empty container if prop on object', () => {
        const container: Maybe = new Maybe({ name: 'red', age: 10 })
        const newContainer: Maybe = container
            .map(prop('age'))
            .map(add(10))
            .map(add(10))
    ...
    it('should return empty container if prop not on object', () => {
        const user = { name: 'red' }
        const getAgeAndAddTen: (user: object) => Maybe = compose(
            map(add(10)),
            safeProp('age')
        )
